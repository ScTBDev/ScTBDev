...
class sql_database_mysql extends sql_database {
	...
	public function connect() {
		...
		if ($this->_persistent)
			$this->db = mysql_pconnect($this->_host, $this->_user, $this->_pass);
		else
			$this->db = mysql_connect($this->_host, $this->_user, $this->_pass);
		...
		elseif (!mysql_select_db($this->_db, $this->db)) {
			$this->set_error();
			trigger_error('Error seleting MySQL DB in '.__CLASS__.'::'.__METHOD__.
				($this->errno ? ': '.$this->errno.' ('.$this->error.')' : ''), E_USER_WARNING);
			return false;
		}
		...
		if (!$result) {
			switch ($this->errno) {
				...
				case 1062:
					...
					trigger_error('SQL ERROR in '.__CLASS__.'::'.__METHOD__.' '.$this->errno.' ('.$this->error.'): '.$sql, E_USER_NOTICE);
					break;
				default:
...
class sql_result_mysql extends sql_result {
	...
	public function __construct($result) {
		if (!is_resource($result)) {
			trigger_error('Argument 1 for '.__CLASS__.'::'.__METHOD__.' must be of type resource, '.
				gettype($result).' given', E_USER_ERROR);
			return;
		}
		if (get_resource_type($result) !== 'mysql result') {
			trigger_error('Argument 1 for '.__CLASS__.'::'.__METHOD__.' must be resource type "mysql result", '.
